...
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#
##### PCA prestige correlation matrix #####
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#
...
## dynamic panel - COHA - w/ lagged independent variable
data <- full_data[which(full_data$corpus=="COCHA"),] %>% ungroup()
data_dpm <- panel_data(data %>% mutate(year = (year-1890)/10), id = id, wave = year)
for (i in c("prestige","potency","evaluation","activity")){
...
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~
##### Longitudinal Analysis with Interaction #####
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~
...
## COCHA
COCHA <- read.csv("COCHA_bigram.csv")
COCHA <- COCHA %>%
  rename(occupation_origin = Occupation..1950.basis) %>%
  distinct(occupation_origin, year, .keep_all = T)

## merge with 1950 occupation official code
code <- read.csv("1950occcode.csv")
code$occ <- gsub('n.e.c.', 'nec', code$occ)
COCHA$occupation_origin <- gsub('n.e.c.', 'nec', COCHA$occupation_origin)
COCHA <- merge(COCHA, code, by.y="occ", by.x="occupation_origin", all.x = T)
rm(code)

## merge with 1950 occupation actual measures
occ1950 <- read.csv("occ1950.csv")
COCHA <- merge(COCHA,occ1950,
                   by.x=c("code","year"),
                   by.y=c("occ1950","year"),
                   all.x=T)
COCHA[which(COCHA$hwsei90 < -10000 | COCHA$hwsei90 > 10000),"hwsei90"] <- NA
COCHA[which(COCHA$prent90 < -10000 | COCHA$prent90 > 10000),"prent90"] <- NA
...
for (out in c("prestige","potency","evaluation","activity")){
  for (t in 1:5){
    results_COCHA[results_COCHA$outcome==out&results_COCHA$window==t,"coef"] <- 
      FErobust(COCHA, count=12, t=t, outcome=out,
               treatment="gender",control=c("education","income"))[1]
    results_COCHA[results_COCHA$outcome==out&results_COCHA$window==t,"se"] <- 
      FErobust(COCHA, count=12, t=t, outcome=out,
               treatment="gender",control=c("education","income"))[2]
...
results_COCHA %>%
  ...
  geom_hline(yintercept=0, linetype="dashed", color = "grey50") +
  theme_minimal() +
  scale_x_discrete(labels= c("full","1","2","3","4","5")) +
  facet_grid(.~outcome) +
  ylim(-0.22,0.1) +
  ggtitle("COCHA Bi-Gram, 1900-2019") +
  scale_color_manual(values=c("blue3","red4")) +
  xlab("parallel trend window (decades)") +
  ylab("coefficient") +
  theme(legend.position = "none",
...
t <- full_data %>% filter(corpus=="COCHA"&year==2010&
                            education<0&income>0)

#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#
##### Count Weighted Occupation #####
#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#~#
