...
def get_pca(embeddings, dimension, category: str):
    """
    Retrieves the PCA of vectors of each category
    """
    ...
    ## create difference for each antonym pair
    difference = embeddings[list(chain(*dimension[schema_words_dict[category][1]]))] - \
    embeddings[list(chain(*dimension[schema_words_dict[category][0]]))]
    ## drop nan
    difference = difference[~np.isnan(difference).any(axis=1),]
    ## standardize the features (midpoint aligned)
    scaler = StandardScaler()
    scaler.fit(difference.T)
    difference = scaler.transform(difference.T)
    ## PCA
    pca_principal = PCA(n_components=1,random_state=2023)
    pca_principal.fit(difference)
    pca = pca_principal.transform(difference)
    return pca.T
